package com.wb.base.callback.consumer.controller;
    ...
    public String test(@Validated @RequestBody String param) {
        return CallbackClient.getInstance().callAndConsume(
                ...
                (CallBackConsumer<TestMessage>) message -> {
                    System.out.println("HTTP消费 ： " + WbJSON.toJson(message) + " , 平台 ： " + message.getPlatformType() + " , 业务场景 ： " + message.getBusinessType() + " , 消息实体 ： " + WbJSON.toJson(message.getData()));
                    return true;
                }
        );
    }
