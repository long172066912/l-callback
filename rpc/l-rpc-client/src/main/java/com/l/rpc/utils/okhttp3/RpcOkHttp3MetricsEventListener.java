 ...
 */
package com.l.rpc.utils.okhttp3;
 ...
 * doesn't provide URI patterns. We provide {@value RpcOkHttp3MetricsEventListener#URI_PATTERN} header to support
...
@NonNullApi
@NonNullFields
public class RpcOkHttp3MetricsEventListener extends EventListener {
    ...
    protected RpcOkHttp3MetricsEventListener(MeterRegistry registry, String requestsMetricName, Function<Request, String> urlMapper,
    ...
    RpcOkHttp3MetricsEventListener(MeterRegistry registry, String requestsMetricName, Function<Request, String> urlMapper,
                                   Iterable<Tag> extraTags,
                                   Iterable<BiFunction<Request, Response, Tag>> contextSpecificTags,
    ...
    void time(CallState state) {
        ...
        String statusMessage = getStatusMessage(state.response, state.exception);
        Iterable<Tag> tags = Tags.of(
                        "method", requestAvailable ? request.method() : TAG_VALUE_UNKNOWN,
                        "uri", getUriTag(state, request),
                        "status", statusMessage,
                        "result", "200".equals(statusMessage) ? "SUCCESS" : "FAIL"
                )
                .and(extraTags)
    ...
    private String getUriTag(CallState state, @Nullable Request request) {
        ...
        return state.response != null && (state.response.code() == 404 || state.response.code() == 301)
                    ? "NOT_FOUND" : request.url().uri().getPath();
    }
    ...
    public static class Builder {
        ...
        /**
         * Historically, OkHttp Metrics provided by {@link RpcOkHttp3MetricsEventListener} included a
        ...
        public RpcOkHttp3MetricsEventListener build() {
            return new RpcOkHttp3MetricsEventListener(registry, name, uriMapper, tags, contextSpecificTags, requestTagKeys, includeHostTag);
        }
    }
}
